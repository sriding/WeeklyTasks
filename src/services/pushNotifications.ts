...
const unCheckingATaskNotification = async (taskId: number): Promise<void> => {
  try {
    await removeALocalScheduledNotification(taskId);
...
const updateADailyRepeatingNotification = async (
  ...
  try {
    ...
    if (currentDay < savedDay) {
      PushNotification.localNotificationSchedule({
        /* Android Only Properties */
        id: (1000000 + dayIndex).toString(), // (optional) Valid unique 32 bit integer specified as string. default: Autogenerated Unique ID
        ticker: "My Notification Ticker", // (optional)
        showWhen: true, // (optional) default: true
        autoCancel: true, // (optional) default: true
        largeIcon: "ic_launcher", // (optional) default: "ic_launcher"
